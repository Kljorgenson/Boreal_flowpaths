...
slope.df2 <- read.csv("Output_data/Slopes.csv")
...
pred.dat %>% ggplot(aes(x, median, color = source)) +
 ...
 theme(legend.position = "top", strip.background =element_rect(fill="white")) +
  ...
  labs(fill = NULL) + guides(linetype = "none") + scale_x_continuous(n.breaks = 3) +
...
pred.dat %>% ggplot(aes(x, median, color = source)) +
  ...
  scale_fill_manual(labels = c("Precipitation", "Soil water", "Vault groundwater", "Groundwater"), 
                     values = c("#66c2a5", "#fc8d62", "#8da0cb", "#e78ac3")) +
  labs(color = NULL, fill = NULL, y = "Proportion", x = "Days since snowmelt") + theme_bw() +
  geom_line(data = pred.int, aes(x,upper/100, color = source), stat = "smooth", linetype = 2) + # Divide by 100 because they were calculated as percentages
  geom_line(data = pred.int, aes(x,lower/100, color = source), stat = "smooth", linetype = 2) +
  ...
  guides(color = "none", fill = guide_legend(override.aes=list(alpha = 1))) +
  scale_x_continuous(breaks = c(60, 90, 120))
...
Site_char <- read.csv("Raw_data/Site_characteristics.csv") %>% pivot_longer(cols = 2:6, names_to = "Site") %>% pivot_wider(names_from = Attribute, values_from = value) %>% filter(Site != "CARI")
Site_char$Catchment_area <- Site_char$Catchment_area/100

dat.char.slope <- full_join(Site_char, slope.dat)

# Calculate cumulative precipitation
Precip.dat <- read.csv("Raw_data/Precip_all.csv")
...
slope.dat.char <- full_join(dat.char.slope, Precip.cum) %>% select(Site, Year, Source, Mean_beta, SD_beta, p2.5_beta, p97.5_beta, Precip_cm, Precip_fall_cm, Prop_fall, Catchment_area, Slope, Deciduous, Permafrost)

write.csv(Precip.cum, "Output_data/Cumulative_precip_calc.csv", row.names = F)
...
slope.dat.char   %>% filter(is.na(Source) ==F, Source %in% c("upwelling", "well")) %>%
  ggplot() + geom_point(aes(Prop_fall, Mean_beta, color = Source, shape = Year)) +
  facet_wrap(~Site, scales = "free_y", labeller = labeller(Site = site_labs)) + 
  xlab("Proportion of rain in autumn") + ylab("Seasonal change in \n groundwater contribution (%/day)") +
  ...
  theme(strip.background =element_rect(fill="white"), legend.position = c(0.85,0.22), legend.key.spacing.y = unit(0,'cm'),
        panel.grid.major = element_blank(),legend.key.width = unit(0.26, "cm"),
        plot.margin = unit(c(.5,.5,.5,.5), "cm"), legend.margin = margin(t=0,b=1, l = 0, r=0),legend.box.margin=margin(-10,-10,-10,-10)) +
  labs(color = NULL) +
  geom_ribbon(data = pred.int, aes(x = x, ymin=lower,ymax=upper), alpha=0.3, fill = "#8da0cb") +
  #geom_errorbar(aes(Prop_fall, Mean_beta, ymin=p2.5_beta, ymax=p97.5_beta, color = Source), width=.1, size = .5) +
  guides(shape = guide_legend(position = 'top', nrow = 1)) 


ggsave("Figures/Groundwater slope and fall rain SD.png", width = 4.5, height = 3.2)
