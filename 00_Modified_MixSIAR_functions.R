##### These TWO functions are modified from MixSIAR code plot_continuous_var.R: https://github.com/brianstock/MixSIAR



### Function to extract predicted lines from MixSIAR model
plot_continuous_var_2 <- function(jags.1, mix, source, output_options, alphaCI=0.05, exclude_sources_below=0.1){
...
######## Function for extracting regression lines from mixing model output  
#### This function is modified from MixSIAR's plot_continuous_var.R script.
# This code is limited to working for models with no fixed/random effects except for one continuous variable.


continuous_var <- function(jags.1, mix, source, output_options, alphaCI=0.05, exclude_sources_below=0.1){
  # added only to pass R CMD check
  # ilr.global <- x <- p.global <- p.ind <- sources <- ..scaled.. <- NULL
  R2jags::attach.jags(jags.1)
  n.sources <- source$n.sources
