...
k <- "MOOS_2022"
mix_d <- all_dat %>% filter(Site == "MOOS", year == "2022") %>% select(Days_melt, Chloride_uM, Magnesium_uM) %>% 
  group_by(Days_melt) %>% summarise(Chloride_uM = mean(Chloride_uM, na.rm = TRUE),
