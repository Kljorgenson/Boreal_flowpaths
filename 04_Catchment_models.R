...
# % Permafrost model
jagsdata <- with(well, list(y = Mean, car = Permafrost, year = Year, Ysd = SD,
                            n = length(Mean), Nyears = Nyears))
...
# % Permafrost model
jagsdata <- with(ppt, list(y = Mean, car = Permafrost, year = Year, Ysd = SD,
                           n = length(Mean), Nyears = Nyears))
...
write.csv(catchment_mods, "Tables/Catchment_characteristics_models.csv", row.names = F)
...
ends <- dat.char[,c(1,2,10, 11,12,13,14,16,17)] %>% pivot_longer(cols = 6:9, names_to = "var") %>% 
  group_by(var, Source) %>% 
...
# % Permafrost
p3 <- dat.char %>% filter(Site != "VAUL") %>% ggplot(aes(Permafrost, Mean, color = Source)) + geom_point(cex = 2) +
  theme_bw() + xlab("Permafrost extent (%)")  + ylab(NULL)   + scale_color_manual(labels = c("Precipitation", "Soil water", "Groundwater"), 
                                                                                values = c("#66c2a5", "#fc8d62", "#8da0cb")) +
  geom_line(data = pred.int[pred.int$var == "Permafrost" & pred.int$source == "well",], aes(x, median, color = source), stat="smooth") +
  geom_line(data = pred.int[pred.int$var == "Permafrost" & pred.int$source == "well",], aes(x,upper, color = source), stat="smooth", alpha = 0.5) +
  geom_line(data = pred.int[pred.int$var == "Permafrost" & pred.int$source == "well",], aes(x,lower, color = source), stat="smooth", alpha = 0.5)  +
  ylim(0,0.9)+ labs(col = NULL)


# Catchment size
p4 <- dat.char %>% filter(Site != "VAUL") %>% ggplot(aes(Catchment_area, Mean, color = Source)) + geom_point(cex = 2) +
  theme_bw() + xlab(expression(Catchment~area~(km^2)))  + ylab(NULL)   + scale_color_manual(labels = c("Precipitation", "Soil water", "Groundwater"), 
                                                                                         values = c("#66c2a5", "#fc8d62", "#8da0cb"))+
...
ggsave('Figures/Catchment models.png', width = 3.8, height = 4, bg = 'white')
...
args <- expand.grid( b = 1:5, a = 1:length(turnout.out$a1)) # Expand combinations
...
ggsave("Figures/Cumulative precip and proportions.png", width = 4.5, height = 3.5)
